...
class JoinRequestService {
    ...
    async create(
        ...
        // Используем глобальный перехватчик для добавления токена
    ...
    subscribeToCompanyRequests(
        ...
        window.Echo.private(`company.${companyId}`).listen(
    ...
    unsubscribeFromCompanyRequests(companyId: number): void {
        window.Echo.private(`company.${companyId}`).stopListening(
            ".join-request.updated"
        );
    }
    ...
    unsubscribeFromUserRequests(userId: number): void {
        window.Echo.private(`user.${userId}`).stopListening(
            ".join-request.updated"
        );
    }
}
